...
d3.json("data_v1.json", function(error, graph) {
  ...
  force.on("tick", function() {
    link.attr("x1", function(d) {
        return Math.min(Math.max(d.source.x, 30), height - 30);
      })
      .attr("y1", function(d) {
        return Math.min(Math.max(d.source.y, 30), width - 30);
      })
      .attr("x2", function(d) {
        return Math.min(Math.max(d.target.x, 30), height - 30);
      })
      .attr("y2", function(d) {
        return Math.min(Math.max(d.target.y, 30), width - 30);
      });

    node.attr("cx", function(d) {
        return Math.min(Math.max(d.x, 30), width - 30);
      })
      .attr("cy", function(d) {
        return Math.min(Math.max(d.y, 30), height - 30);
      });
